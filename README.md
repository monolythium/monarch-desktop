...
- **Welcome onboarding (`/welcome`)** — one persistent checklist from a freshly provisioned node to a signing cluster seat: connect + provision the node → create or import the operator key (in-app generation, shown once) → fund the 5,000 LYTH bond (copy the funding address, then request the bond from the team/foundation in the testnet [Discord](https://discord.gg/monolythium) — a deliberate manual gate, not a self-service faucet) → register → set an operator name → publish the seal key → publish chat peers → join or form a cluster. Step state is **detected** from the node, the keychain, and the chain — not remembered — so progress survives restarts and never lies.
...
`pnpm install` resolves all dependencies from npm or public GitHub package sources, including the pinned `@monolythium/core-sdk` commit in [`monolythium/mono-core-sdk`](https://github.com/monolythium/mono-core-sdk). No sibling checkout is required.
...
cluster id, and live operator chat metadata current for each
candidate.
