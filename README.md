...
- **SDK is consumed straight from npm.** `package.json` pins `@monolythium/core-sdk` to the exact published version `0.3.11` ([`monolythium/mono-core-sdk`](https://github.com/monolythium/mono-core-sdk)). `pnpm install` resolves it from the registry — no sibling checkout required.
...
`pnpm install` resolves all dependencies from npm — including `@monolythium/core-sdk@0.3.11` ([`monolythium/mono-core-sdk`](https://github.com/monolythium/mono-core-sdk)). No sibling checkout is required.
